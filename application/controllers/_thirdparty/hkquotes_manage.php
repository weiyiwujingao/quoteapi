    ...
    {   
		...
		if(is_empty($stockInfo))
		{
			foreach($stockInfo['list'] as $key => $rs)
			{
				...
				/* 成交总额 */
				$this->data[$key]['Amt']		   = formats($rs[32165]/10000, 2);
				/* 成交总量 */
				$this->data[$key]['Vol']		   = formats($rs[32165], 2);
				/* 换手率 */
				$this->data[$key]['VolChangeRate'] = formats($rs[32175], 2);
				/* 市盈率 */
				$this->data[$key]['EPS']		   = formats($rs[32204], 2);
				/* 内盘 */
				$this->data[$key]['InVol']		   = formats($rs[32171]/10000, 2);
				/* 外盘 */
				$this->data[$key]['OutVol']		   = formats($rs[32172]/10000, 2);
			}
	...
	{  
		...
		if(is_empty($stockInfo))
		{
			...
			foreach($stockInfo['list'] as $key => $rs)
			{
				...
				$this->data['list'][$key]['Amt']		   = formats($rs[32166]/10000, 2);
				/* 成交总量(万) */
				$this->data['list'][$key]['Vol']		   = formats($rs[32165], 2);
				/* 换手率 */
				$this->data['list'][$key]['VolChangeRate'] = formats($rs[32175]*100, 2);
				/* 振幅 */
				$this->data['list'][$key]['HLDiffRate']	   = formats($rs[32170], 2);
				/* 市盈率 */
				$this->data['list'][$key]['EPS']		   = formats($rs[32204], 2);
			...
			if($type)
			{
				...
				if(is_empty($stockInfo))
				{
					foreach($stockInfo['list'] as $key => $rs)
					{
						...
						$this->data['list'][$key]['StockID2']      = $rs[32101];
						/* 股票名称 */
						$this->data['list'][$key]['StockName']     = replace($rs[32102]);
						/* 最新价 */
						$this->data['list'][$key]['ClosePrice']    = formats($rs[32154]/1000, 3);
						/* 涨跌额 */
						$this->data['list'][$key]['DiffPrice']     = formats($rs[32168], 2);
    ...
    private function getAhList() 
    {
		...
		if(is_empty($stockInfo))
		{
            ...
            foreach($stockInfo['list'] as $key => $rs)
            {
				...
				$this->data['list'][$key]['DiffPriceRate'] = formats($rs[32169], 2);
				/* 成交量 */
				$this->data['list'][$key]['Vol']		   = formats($rs[32163], 2);
				...
				/* 最新价 */
				$this->data['list'][$key]['HKClosePrice']    = formats($rs[32182]/1000, 2);
				/* 涨跌额 */
				$this->data['list'][$key]['HKDiffPrice']     = formats($rs[32177], 2);
				/* 涨跌幅 */
				$this->data['list'][$key]['HKDiffPriceRate'] = formats($rs[32208], 2);
				/* 成交量 */
				$this->data['list'][$key]['HKVol']			 = formats($rs[32165], 2);
				/* AH比价 */
				$this->data['list'][$key]['HKPriceRate']     = formats($rs[32183], 2);
				/* AH量比 */
				$this->data['list'][$key]['HKVolRate']       = formats($rs[32174], 2);
            }
			unset($stockInfo);
		}
		$offset = ($page - 1) * $limit;
