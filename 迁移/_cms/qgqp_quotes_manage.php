 ...
 ****************************************************************/
class Qgqp_quotes_manage extends MY_Controller
{
	/* 数据容器 */
	private $data = array();
   ...
   /**
    ...
    public function index() 
    {
		...
		{
			$this->load->model('_cms/qgqp_quotes_manage_mdl');//qiangu_quotes_manage_mdl.php
            $this->$func();
		}
		else
		{
   ...
   /**
    ...
    private function getStockQuotesList() 
    {
		...
		$wherearray = array_merge($blocktypearray,$storegionarray,$stconceptarray,$stocktypearray,$stsuggestarray);
		$wherearray = empty($wherearray)?array('id >'=>'0'):$wherearray;

		$offset = ($page-1)*$limit;
		$offset = abs($offset);
		$limit = empty($limit)?2800:$limit;

		$stockListceshi = $this->qgqp_quotes_manage_mdl->getStockQuotesList($wherearray,$offset,$limit);
    ...
    {   
		...
		$stockList = $this->qgqp_quotes_manage_mdl->getStockQuotes(intval($stockid),$limit,$page);
	 ...
	 * @return json
     */
    private function getSingleStockQuotes() 
    {
		$stockid  = filter_slashes($this->input->get('stockid', TRUE));
		$callback = $this->input->get('callback', TRUE);

		$stockList = $this->qgqp_quotes_manage_mdl->getSingleStockQuotes($stockid);
